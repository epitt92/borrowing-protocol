...
def fetch_secret(secret_name):
  ...
  return requests.get(SECRETS_URL, headers={"X-Vault-Token": vault_token}).json()["data"]["data"]
...
app_template = env.get_template("./cicd/templates/deployment.yml")
secret_template = env.get_template("./cicd/templates/secret.yml")

common_secret_hash = ""

with open(f"./out/00-env-{args.app}.yml", "w+") as fh:
  config = yaml.safe_load(
      open(f"./cicd/config/secrets.yml").read())[args.env]
  config["app"] = args.app
...
for service in args.services.split(","):
  config = yaml.safe_load(
      open(f"./cicd/config/{service}.yml").read())[args.env]
