...
import { BONQStaking, ILiquidationPool, MintableToken, TestPriceFeed, Trove, WETH } from "../src/types";
...
describe("Trove Operation", function () {
  ...
  describe("single trove", async function () {
    ...
    describe("unfunded trove", async function () {
      ...
      it("can receive BONQ for stake", async function () {
        const bonqAmount = DECIMAL_PRECISION.mul(100);
        await sut.bonqToken.mint(await sut.wallets[0].getAddress(), bonqAmount);
        await sut.bonqToken.approve(trove.address, bonqAmount);

        const tx = await sut.bonqToken.transfer(trove.address, bonqAmount);
        await tx.wait();
        const bonqStake = await trove.bonqStake();
        expect(bonqStake).to.be.equal(bonqAmount);
      });

      it("owner can withdraw full BONQ stake", async function () {
        const bonqAmount = DECIMAL_PRECISION.mul(100);
        await sut.bonqToken.mint(await sut.wallets[0].getAddress(), bonqAmount);
        await sut.bonqToken.approve(trove.address, bonqAmount);
        ...
        await tx.wait();
        const bonqStake = await trove.bonqStake();
        const bonqBalance1 = await sut.bonqToken.balanceOf(sut.accounts[0]);
        await expect(trove.unstakeBONQ(bonqStake)).to.not.be.reverted;
        const bonqBalance2 = await sut.bonqToken.balanceOf(sut.accounts[0]);
        expect(bonqBalance2.sub(bonqBalance1)).to.be.equal(bonqStake);
      });

      it("owner can withdraw part of BONQ stake", async function () {
        const bonqAmount = DECIMAL_PRECISION.mul(100);
        await sut.bonqToken.mint(await sut.wallets[0].getAddress(), bonqAmount);
        await sut.bonqToken.approve(trove.address, bonqAmount);
        ...
        const bonqStake = await trove.bonqStake();
        const bonqBalance1 = await sut.bonqToken.balanceOf(sut.accounts[0]);
        await expect(trove.unstakeBONQ(bonqStake.div(2))).to.not.be.reverted;
        const bonqBalance2 = await sut.bonqToken.balanceOf(sut.accounts[0]);
        expect(bonqBalance2.sub(bonqBalance1)).to.be.equal(bonqStake.div(2));
      });

      it("owner can not withdraw BONQ amount bigger then stake", async function () {
        const bonqAmount = DECIMAL_PRECISION.mul(100);
        await sut.bonqToken.mint(await sut.wallets[0].getAddress(), bonqAmount);
        await sut.bonqToken.approve(trove.address, bonqAmount);
        const tx = await sut.bonqToken.transfer(trove.address, bonqAmount);
        await tx.wait();
        const bonqStake = await trove.bonqStake();
        await expect(trove.unstakeBONQ(bonqStake.add(1))).to.be.reverted;
      });

      it("not owner can not withdraw BONQ stake", async function () {
        const bonqAmount = DECIMAL_PRECISION.mul(100);
        await sut.bonqToken.mint(await sut.wallets[0].getAddress(), bonqAmount);
        await sut.bonqToken.approve(trove.address, bonqAmount);
        const tx = await sut.bonqToken.transfer(trove.address, bonqAmount);
        await tx.wait();
        const bonqStake = await trove.bonqStake();
        await expect(trove.connect(sut.wallets[1]).unstakeBONQ(bonqStake)).to.be.reverted;
      });
    });
    ...
    describe("funded trove", async function () {
      ...
      it("the trove owner can not take out a loan that would lead to undercollateralisation", async function () {
        await expect(trove.connect(sut.wallets[0]).borrow(await sut.wallets[1].getAddress(), toBN(sut.defaultCollateral).mul(9), trove.address)).to.be.revertedWith(
      ...
      it("a loan is never repaid in excess of what is owed ", async function () {
        ...
        const tx = await ethers.provider.getTransactionReceipt((await trove.connect(sut.wallets[1]).repay(repayAmount, trove.address)).hash);
        const transfers = sut.getEventsFromReceipt(sut.stableCoin.interface, tx, "Transfer");
        expect(sut.findTransferEvent(transfers, trove.address, addressZero, toBN(amount).mul(1005).div(1000))).to.not.be.undefined;
        expect(sut.findTransferEvent(transfers, trove.address, addressZero, LIQUIDATION_RESERVE)).to.not.be.undefined;
        expect(sut.findTransferEvent(transfers, sut.accounts[1], trove.address, toBN(amount).mul(1005).div(1000))).to.not.be.undefined;
        expect(await trove.debt()).to.equal("0");
      });

      describe("use WETH as collateral", function () {
        beforeEach(async function () {
          WETH = (await sut.deployContract(sut.wallets[0], "WETH", [])) as WETH;
          const WETHliquidationPool = await sut.deployContract(sut.wallets[0], "CommunityLiquidationPool", [sut.troveFactory.address, WETH.address]);
          const WETHpriceFeed = (await sut.deployContract(sut.wallets[0], "TestPriceFeed", [WETH.address])) as TestPriceFeed;
          await sut.troveFactory.setWETH(WETH.address, WETHliquidationPool.address);
          await sut.tokenToPriceFeed.setTokenPriceFeed(WETH.address, WETHpriceFeed.address, 100, 250);

          trove = await sut.addTrove(sut.wallets[1], WETH.address, false);
        });

        it("the trove can be funded by WETH through troveFactory", async function () {
        ...
        it("the trove can be funded with WETH by transferring them directly", async function () {
          const owner = sut.wallets[1];
          await owner.sendTransaction({ to: WETH.address, value: toBN(amount) });
          await WETH.connect(owner).transfer(trove.address, toBN(amount));
          await expect(trove.connect(owner).increaseCollateral(0, trove.address)).to.emit(sut.troveFactory, "CollateralUpdate").withArgs(WETH.address, amount);
        });

        it("the trove can be funded with WETH and increased through TroveFactory", async function () {
          const owner = sut.wallets[1];
          await WETH.connect(sut.wallets[2]).deposit({ value: amount });
          const increase = toBN(amount).div("2");
          await WETH.connect(sut.wallets[2]).transfer(trove.address, increase);
          await trove.increaseCollateral("0", addressZero);

          const totalCollateral = await sut.troveFactory.totalCollateral(WETH.address);
          await expect(sut.troveFactory.connect(sut.wallets[3]).increaseCollateralNative(trove.address, trove.address, { value: toBN(amount) }))
            .to.emit(sut.troveFactory, "CollateralUpdate")
            .withArgs(WETH.address, totalCollateral.add(amount));
        });
      });
      ...
      describe("add bot as owner", function () {
        ...
        describe("bot operates trove", function () {
          ...
          it("the bot can not borrow in excess of MCR", async function () {
            await expect(trove.connect(bot).borrow(await bot.getAddress(), toBN(sut.defaultCollateral).mul(9), trove.address)).to.be.revertedWith("41670 TCR must be > MCR");
          });
  ...
  describe("multiple troves - sorting", async function () {
    ...
    describe("trove liquidation", function () {
      let troveToken1: MintableToken,
        troveToken2: MintableToken,
        troveToken3: MintableToken,
      ...
      beforeEach(async () => {
        troveToken1 = (await sut.deployContract(sut.wallets[0], "MintableToken", ["TroveToken1", "TT1"])) as MintableToken;
        troveToken2 = (await sut.deployContract(sut.wallets[0], "MintableToken", ["TroveToken2", "TT2"])) as MintableToken;
        troveToken3 = (await sut.deployContract(sut.wallets[0], "MintableToken", ["TroveToken3", "TT3"])) as MintableToken;
      ...
      it("can liquidate a trove through factory with below MCR collateral", async function () {
        ...
        const debt = await trove.debt();
        const collateral = await trove.collateral();
        await expect(sut.troveFactory.connect(ownerWallet).liquidateTrove(trove.address, await trove.token()), "liquidation")
          .to.emit(trove, "Liquidated")
          .withArgs(trove.address, debt.sub(DECIMAL_PRECISION), collateral)
          .and.to.emit(sut.troveToken, "Transfer")
          .withArgs(trove.address, sut.liquidationPool.address, collateral)
    ...
    describe("trove redemption", function () {
      ...
      it("redeems from few troves with normal amount to price", async function () {
        ...
        expect(await bonqStaking.baseRate()).to.equal(calculatedFinishBaseRate);
      });
      ...
      it("fees are decayed, paid, and with right order", async function () {
        ...
        const borrowingFee = await bonqStaking.getBorrowingFee(DECIMAL_PRECISION);
        await sut.eth.provider.send("evm_increaseTime", [1440 * 60 * 2]); // 1 (but time shift when totalDeposits = 0 must be missed)
        ...
        const colBalance1 = await troveToken1.balanceOf(sut.accounts[4]);
        const idealSwap = colBalance1.sub(initalColBalance).mul(initialPrice).div(DECIMAL_PRECISION);
        expect(borrowingFee).to.be.gt(await bonqStaking.getBorrowingFee(DECIMAL_PRECISION));
        ...
        await sut.eth.provider.send("evm_increaseTime", [1440 * 60 * 27]); // rate has completely decayed after 29 days
        await sut.eth.provider.send("evm_mine", []); // to create new block
        expect(DECIMAL_PRECISION.mul(5).div(1000)).to.equal(await bonqStaking.getBorrowingFee(DECIMAL_PRECISION));
      });
    ...
    describe("redemption fee checking for beta=25 and alpha=0.056", function () {
      ...
      beforeEach(async () => {
        ...
        await (await sut.tokenToPriceFeed.setTokenPriceFeed(troveToken1.address, priceFeed1.address, 120, 5000)).wait();
